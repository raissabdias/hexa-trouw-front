...
export default function TravelIndexPage() {
  ...
  const validMarkers = invoices.filter((inv) => {
  ...
  useEffect(() => {
    if (mapInstance && validMarkers.length > 0 && window.google) {
      const bounds = new window.google.maps.LatLngBounds();
      validMarkers.forEach((inv) => {
        const lat = Number(inv.recipient?.address?.latitude);
        const lng = Number(inv.recipient?.address?.longitude);
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 min-h-0">
      ...
      <div className="flex flex-1 min-h-0 flex-col overflow-hidden rounded-lg bg-white shadow-sm ring-1 ring-zinc-200">
        <div className="border-b border-zinc-200 bg-zinc-50 px-4 py-3 shrink-0">
          <h2 className="text-sm font-medium text-zinc-900">
            Visualização de Notas ({invoices.length} carregadas)
          </h2>
        </div>
          ...
          <table className="min-w-max divide-y divide-zinc-200">
            ...
            <tbody className="divide-y divide-zinc-200">
              {loading ? (
                ...
                <tr>
                  <td colSpan={13} className="px-4 py-10 text-center text-sm text-red-600">
                    {error}
                  </td>
                </tr>
              ) : invoices.length === 0 ? (
